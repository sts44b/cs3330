...
public class Grid {
	...
	//return the name of the server at the index indicated
	public String getServerName (int server_idx){
		if (server_idx > 0 && server_idx <= servers.size()){
			return this.servers.get(server_idx).getName();
		}
		
		//if the server was not fount inform the user
		else{
